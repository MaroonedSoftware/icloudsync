...
The cache and download proxy now cover video and non-JPEG renditions too, not just JPEG thumbnails: every derived rendition is served with the correct MIME (`resVid*` clips as `video/mp4`, other formats via their CloudKit `fileType` UTI), so a video preview plays inline instead of downloading. Video tiles get a ▶ play badge in the recent-backups grid. Renditions larger than the whole cache budget (big clips) are served but not cached, so they can't churn the cache.
