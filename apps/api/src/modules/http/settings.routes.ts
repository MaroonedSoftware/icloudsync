...
/**
 * Per-account overrides for the on-disk photo organization. Any field may be
 * `null` to clear the override (fall back to the default) or a valid value to pin
 * it; at least one must be present.
 */
 ...
 *
 * The on-disk photo organization (filesystem preset + layout/naming) is
 * per-account. Per-account overrides inherit the built-in defaults (`immich`
 * preset, its flat/clean baseline) or the Apple ID's local part (archive prefix)
 * unless pinned, keyed by account id in the path:
 * - `GET /icloud/accounts/:accountId/settings` → `{ photosPreset, photosLayout,
 *   photosNaming, archivePrefix, defaultPrefix, defaults }` — the account's
 *   overrides (`null` = inherit) plus the defaults they fall back to.
 * - `PATCH /icloud/accounts/:accountId/settings` `{ photosPreset?, photosLayout?,
 *   photosNaming?, archivePrefix? }` — a value pins the override, `null` (or `''`
 *   for the prefix) clears it; returns the same shape as GET.
 */
export function icloudSettingsRouter() {
    const router = ServerKitRouter();
    const json = bodyParserMiddleware(['application/json']);

    /** The account-settings view (preset + layout/naming overrides, the defaults they inherit, and live relocation state). */
    const accountSettingsView = async (accountId: string, container: Container) => {
        ...
        const [override, account, relocatingId] = await Promise.all([
        ...
        return {
            ...
            // The values a null override falls back to (built-in preset + the preset baseline).
            defaults: {
