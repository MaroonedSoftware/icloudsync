...
describe('AccountsService photo overrides (integration)', () => {
    it('defaults to inheriting (all null), then pins and clears overrides', async () => {
        ...
        expect(await accounts.photoSettings(id)).toEqual({ preset: null, layout: null, naming: null });

        // Pin the preset and the layout; naming stays inherited.
        await accounts.setPhotoSettings(id, { preset: 'browsable', layout: 'date' });
        expect(await accounts.photoSettings(id)).toEqual({ preset: 'browsable', layout: 'date', naming: null });
        ...
        expect(await fresh.photoSettings(id)).toEqual({ preset: 'browsable', layout: 'date', naming: 'hash' });

        // Clearing the layout leaves the rest intact.
        await accounts.setPhotoSettings(id, { layout: null });
        expect(await fresh.photoSettings(id)).toEqual({ preset: 'browsable', layout: null, naming: 'hash' });
    });
    ...
    it('reads an unknown account as fully inherited', async () => {
        ...
        expect(await accounts.photoSettings('00000000-0000-4000-8000-000000000000')).toEqual({ preset: null, layout: null, naming: null });
    });
});
