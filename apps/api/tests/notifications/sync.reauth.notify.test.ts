...
const accounts = {
    ...
    photoSettings: async () => ({ preset: null, layout: null, naming: null }),
};
...
describe('SyncPhotosJob reauth notifications', () => {
    it('notifies the admin (with the Apple ID) when an account cannot be restored', async () => {
        ...
        const job = new SyncPhotosJob(source(false), store, archive, silentLogger, reauth, accounts);
    ...
    it('clears the throttle when an account is authenticated', async () => {
        const reauth = new SpyReauth();
        const job = new SyncPhotosJob(source(true), store, archive, silentLogger, reauth, accounts);
